...
struct PortSetup
{
  ...
  static void load(const rapidjson::Value::ConstArray& inlets, const rapidjson::Value::ConstArray& outlets, T& self)
  {
    ...
    for ([[maybe_unused]] const auto& in : Node_T::Metadata::audio_ins)
    {
      ins.push_back(deserialize_known_interface<Process::AudioInlet>(JSONObjectWriter{inlets[inlet++]}, &self));
    }
    for ([[maybe_unused]] const auto& in : Node_T::Metadata::midi_ins)
    {
      ins.push_back(deserialize_known_interface<Process::MidiInlet>(JSONObjectWriter{inlets[inlet++]}, &self));
    }
    for ([[maybe_unused]] const auto& in : Node_T::Metadata::value_ins)
    {
      ins.push_back(deserialize_known_interface<Process::ValueInlet>(JSONObjectWriter{inlets[inlet++]}, &self));
    }
    for ([[maybe_unused]] const auto& in : Node_T::Metadata::address_ins)
    {
      ins.push_back(deserialize_known_interface<Process::ValueInlet>(JSONObjectWriter{inlets[inlet++]}, &self));
    ...
    ossia::for_each_in_tuple(Node_T::Metadata::controls,
                             [&](const auto& ctrl) {
      if (auto p = ctrl.create_inlet(JSONObjectWriter{inlets[inlet++]}, &self))
      {
        p->hidden = true;
        ins.push_back(p);
      }
    ...
    for ([[maybe_unused]] const auto& out : Node_T::Metadata::audio_outs)
    {
      outs.push_back(deserialize_known_interface<Process::AudioOutlet>(JSONObjectWriter{outlets[outlet++]}, &self));
    }
    for ([[maybe_unused]] const auto& out : Node_T::Metadata::midi_outs)
    {
      outs.push_back(deserialize_known_interface<Process::MidiOutlet>(JSONObjectWriter{outlets[outlet++]}, &self));
    }
    for ([[maybe_unused]] const auto& out : Node_T::Metadata::value_outs)
    {
      outs.push_back(deserialize_known_interface<Process::ValueOutlet>(JSONObjectWriter{outlets[outlet++]}, &self));
    }
    ossia::for_each_in_tuple(Node_T::Metadata::control_outs,
                             [&](const auto& ctrl) {
      if (auto p = ctrl.create_outlet(JSONObjectWriter{outlets[outlet++]}, &self))
      {
        p->hidden = true;
        outs.push_back(p);
      }
